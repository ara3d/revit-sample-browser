...
namespace AdnRme
{
    public partial class CmdInspectElectricalForm : WinForm
    {
        ...
        class Loads
        {
            ...
            {
                var loadNode = tn.Nodes.Add("Loads");
        ...
        void PopulateLikeSystemBrowser(IDictionary<string, List<Element>> map)
        {
            ...
            {
                var pair = GetPanelAndSystemPair(panelAndSystem);
                var panelName = pair[0];
                var systemName = pair[1];
                ...
                var elements = map[panelAndSystem];
                List<string> a = new(elements.Count);
        ...
        {
            var elements = _map[panelAndSystem];
            foreach (var e in elements)
            {
                //
                ...
                {
                    var elementNode = tn.Nodes.Add(Util.BrowserDescription(e));
                }
            }
        }
        ...
        {
            var tn = tnc.Add(panelName, panelName);
            AddLoadNodes(tn, e);
            _panelNameToFind = panelName;
            var systemsForPanel = _keys.FindAll(FindPanelBasedOnName);
            systemsForPanel.Sort(new CmdElectricalSystemBrowser.PanelSystemComparer());
            foreach (var panelAndSystem in systemsForPanel)
            {
                var systemNode = tn.Nodes.Add(GetSystemName(panelAndSystem));
        ...
        void PopulateFullHierarchy(IDictionary<string, List<Element>> map)
        {
            _map = map;
            _keys = [.. map.Keys];
            var rootPanels = _keys.FindAll(FindUnassigned);
            foreach (var rootPanel in rootPanels)
            {
                var es = _map[rootPanel];
