...
namespace Ara3D.Bowerbird.RevitSamples
{
    ...
    {
        public static XYZ ToRevit(this Vector3 self)
        {
            return new XYZ(self.X, self.Y, self.Z);
        }

        public static XYZ ToRevit(this Point3D self)
        {
            return new XYZ(self.X, self.Y, self.Z);
        }

        public static UV ToRevit(this Vector2 self)
        {
            return new UV(self.X, self.Y);
        }

        public static UV ToRevit(this Point2D self)
        {
            return new UV(self.X, self.Y);
        }

        public static ColorWithTransparency ToRevit(this Color32 self)
        {
            return new ColorWithTransparency(self.R, self.G, self.B, self.A);
        }

        public static Vector3 ToAra3D(this XYZ self)
        {
            return new Vector3((float)self.X, (float)self.Y, (float)self.Z);
        }

        public static Vector2 ToAra3D(this UV self)
        {
            return new Vector2((float)self.U, (float)self.V);
        }

        public static Line3D ToAra3D(this XYZ self, XYZ other)
        {
            return (self.ToAra3D(), other.ToAra3D());
        }

        public static BoundingBoxXYZ ToRevit(this Bounds3D self)
        {
            return new BoundingBoxXYZ() { Min = self.Min.ToRevit(), Max = self.Max.ToRevit() };
        }

        public static BoundingBoxUV ToRevit(this Bounds2D self)
        {
            return new BoundingBoxUV() { Min = self.Min.ToRevit(), Max = self.Max.ToRevit() };
        }

        //public static Geometry.Plane ToAra3D(this ClipPlane self) => (self.Normal.ToAra3D(), (float)self.Origin.DistanceTo(self.Normal));
        public static Bounds3D ToAra3D(this BoundingBoxXYZ self)
        {
            return new Bounds3D(self.Min.ToAra3D(), self.Max.ToAra3D());
        }

        public static Bounds2D ToAra3D(this BoundingBoxUV self)
        {
            return new Bounds2D(self.Min.ToAra3D(), self.Max.ToAra3D());
        }
        ...
        {
            return new VertexPositionNormalColored(
