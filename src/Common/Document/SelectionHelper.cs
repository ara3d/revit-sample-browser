...
namespace Ara3D.RevitSampleBrowser.Common.Documents
{
    public static class SelectionHelper
    {
        public static bool GetSelectedModelGroup(UIDocument uiDoc, out Group modelGroup, out string errorMessage)
        {
            ...
            if (uiDoc.Document.GetElement(sel.First()) is not Group selectedGroup)
            {
        ...
        {
            LogicalOrFilter types = new([new ElementCategoryFilter(BuiltInCategory.OST_StructConnections)]);
            StructuralConnectionSelectionFilter filter = new(types);
            var target = document.Selection.PickObject(ObjectType.Element, filter, "Select connection element :");
            return target == null ? null : document.Document.GetElement(target) as StructuralConnectionHandler;
        }

        public static List<ElementId> SelectConnectionElements(UIDocument document)
        {
            LogicalOrFilter connElemTypes = new(
            [
            ...
            StructuralConnectionSelectionFilter elemFilter = new(connElemTypes);
        ...
        {
            List<FamilyInstance> beamList = new();
