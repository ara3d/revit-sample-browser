...
namespace Ara3D.RevitSampleBrowser.Common.Geometry
{
    public static class CurveGeometry
    {
        public static XYZ GetTangentAt(Curve curve, double parameter) =>
                    curve.ComputeDerivatives(parameter, true).BasisX.Normalize();

        public static IList<Curve> TransformCurves(IList<Curve> inputs, Transform trf) =>
                    inputs.Select(input => TransformCurve(input, trf)).ToList();

        public static Curve TransformCurve(Curve input, Transform trf) => input.CreateTransformed(trf);

        public static IList<Curve> ProjectCurvesToElevation(IList<Curve> curves, double elevation) =>
                    curves.Select(curve => ProjectCurveToElevation(curve, elevation)).ToList();
        ...
        public static double ComputeParameter(Curve curve, XYZ point) => curve.Project(point).Parameter;
