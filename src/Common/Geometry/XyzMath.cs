...
namespace Ara3D.RevitSampleBrowser.Common.Geometry
{
    public static class XyzMath
    {
        ...
        public static XYZ CrossMatrix(XYZ p1, XYZ p2) =>
            new XYZ(p1.Y * p2.Z - p1.Z * p2.Y, p1.Z * p2.X - p1.X * p2.Z, p1.X * p2.Y - p1.Y * p2.X);
        ...
        public static List<Line> SortLines(List<Line> originLines)
                {
                    ...
                    var result = new List<Line> { lines[0] };
        ...
        public static double GetLength(XYZ vector) => vector.GetLength();

        public static bool IsEqual(XYZ p1, XYZ p2) => CompareXyz(p1, p2);
        ...
        public static XYZ AddXyz(XYZ p1, XYZ p2) => new XYZ(p1.X + p2.X, p1.Y + p2.Y, p1.Z + p2.Z);

        private static XYZ MultiXyz(XYZ p1, double para) => new XYZ(p1.X * para, p1.Y * para, p1.Z * para);
        ...
        private static Plane GetAppropriatePlane(RevitView view) =>
                    Plane.CreateByNormalAndOrigin(view.ViewDirection, view.Origin);
        ...
        public static Curve CalculateAlignedCurve(Curve curve, Curve baseCurve, XYZ baseDirect)
                {
                    ...
                    if (Math.Round(direct.Y) == Math.Round(baseDirect.Y) && Math.Round(direct.Y) == 1)
                    {
                        return Line.CreateBound(
        ...
        public static string XyzToString(XYZ point) => $"( {point.X}, {point.Y}, {point.Z})";
        ...
        public static XYZ GetVector(XYZ startPoint, XYZ endPoint) =>
                    new(endPoint.X - startPoint.X, endPoint.Y - startPoint.Y, endPoint.Z - startPoint.Z);
        ...
        public static Curve CreateReversedCurve(Curve orig)
                {
                    if (!SampleBrowserUtils.SupportsLoopUtilities(orig))
        ...
        public static ModelCurve MakeLine(UIApplication app, XYZ ptA, XYZ ptB) =>
                    MakeLine(app, ptA, ptB, ptA.CrossProduct(ptB).GetLength() == 0 ? XYZ.BasisZ : ptA.CrossProduct(ptB));
        ...
        public static SketchPlane GetSketchPlaneById(Document document, ElementId id)
                {
                    if (!(ElementQuery.GetElementById(document, id) is SketchPlane workPlane))
        ...
        public static bool IsSameDirection(XYZ firstVec, XYZ secondVec) =>
                    IsEqual(DotMatrix(UnitVector(firstVec), UnitVector(secondVec)), 1);
        ...
        public static XYZ MultiplyVector(XYZ vector, double rate) =>
                    new XYZ(vector.X * rate, vector.Y * rate, vector.Z * rate);
        ...
        public static XYZ OffsetPoint(XYZ point, XYZ direction, double offset) =>
                    AddXyz(point, MultiplyVector(UnitVector(direction), offset));
        ...
        public static bool DoubleEquals(double x, double y) => Math.Abs(x - y) < DoubleTolerance;
        ...
        public static XYZ MoveXyzToElevation(XYZ input, double elevation) =>
                    input + XYZ.BasisZ * (elevation - input.Z);
