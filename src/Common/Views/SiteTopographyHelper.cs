...
namespace Ara3D.RevitSampleBrowser.Common.Views
{
    public static class SiteTopographyHelper
    {
        public static void ChangeSubregionAndPointsElevation(UIDocument uiDoc, double elevationDelta)
        {
            ...
            using TopographyEditScope editScope = new(doc, "Raise/lower terrain");
            editScope.Start(toposurface.Id);
            using (Transaction t = new(doc, "Raise/lower terrain"))
            {
                t.Start();
        ...
        {
            return uiDoc.Document.GetElement(uiDoc.Selection.PickObject(ObjectType.Element,
        ...
        {
            return existingPoints.Count == 0 ? 0 : existingPoints.Average(xyz => xyz.Z);
        }
        ...
        {
            return subregion.Document.GetElement(subregion.AsSiteSubRegion().HostId) as TopographySurface;
        }
        ...
        public static List<XYZ> CollectPointsFromImportInstance(ImportInstance import)
        {
            ...
            {
                if (gObject is not GeometryInstance gInstance)
                    continue;
