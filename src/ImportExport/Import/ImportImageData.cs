...
namespace Ara3D.RevitSampleBrowser.ImportExport.CS.Import
{
    public class ImportImageData : ImportData
    {
        ...
        {
            using Transaction t = new(ActiveDoc);
            t.SetName("Import");
            t.Start();
            ...
            ImageTypeOptions typeOptions = new(ImportFileFullName, true, ImageTypeSource.Import);
            ...
            if (ImageInstance.IsValidView(view))
            {
                ...
                ImagePlacementOptions placementOptions = new()
                {
