...
namespace Ara3D.RevitSampleBrowser.StringSearch.CS
{
    public partial class SearchForm : Form
    {
        ...
        readonly string _log_path;
        bool _ok_clicked;

        static readonly List<string> _previous_search_strings
          = [];

        static readonly List<string> _previous_parameter_names
          = [];
        ...
        public SearchForm(string log_path)
        {
            ...
            var wmin = Size.Width - (btnOk.Left - groupBox1.Left);

            var hmin = Size.Height - ClientSize.Height
    ...
    static int[] _minimum_height = new[] { 235, 348, 418, 485, 505 };
      ...
      label3.Visible = _minimum_height[4] < h;
        ...
        {
            var t = typeof(Autodesk.Revit.DB.BuiltInCategory);

            var names = Enum.GetNames(t);

            List<string> bics = new(names.Length);
        ...
        void PopulateBuiltInParameters()
        {
            ...
            var t = typeof(Autodesk.Revit.DB.BuiltInParameter);

            var names = Enum.GetNames(t);

            List<string> bips = new(names.Length);
        ...
        public string ParameterName
        {
            get
            {
                var s = cmbParameter.Text;
                return _all == s ? null : s;
            }
        }

        public bool AllCategories => cmbCategory.Text.Equals(_all);

        public SearchOptions SearchOptions => new(
        ...
        public string SearchString => cmbSearchString.Text;

        public bool CurrentSelection => rbSelection.Checked;

        public bool CurrentView => rbView.Checked;

        public bool ElementType => rbElementType.Checked;

        public bool NonElementType => !rbElementType.Checked;
