...
namespace BuildingCoder
{
    internal static partial class Util
    {
        ...
        ///     automatically.
        /// </summary>
        private static IEnumerable<Element> IterateOverCollector(
            Document doc)
        {
            // Do not do this!
        ...
        private static void TraverseInstances(Document doc)
        {
            var levels
                = new FilteredElementCollector(doc)
        ...
        /// </summary>
        internal static IEnumerable<Room> GetRoomsOnLevel(
            Document doc,
            ElementId idLevel)
        {
        ...
        private static void F(Document doc)
        {
        ...
        private static void F2(Document doc)
        {
        ...
        internal static void TestFilterStringContains(Document doc)
        {
        ...
        private static IEnumerable<IndependentTag> RetrieveRectangularFabricationPartTags(Document doc)
        {
            return new FilteredElementCollector(doc)
                .WhereElementIsNotElementType()
        ...
        /// </summary>
        private static FilteredElementCollector GetPipesForSystemType(
            Document doc,
            string system_name)
        {
        ...
        /// </summary>
        internal static List<string> ListElementsInAssembly(
            Document doc)
        {
        ...
        private static void RetrieveEdges(
            Document doc,
        ...
        private static IEnumerable<Document> CollectorGetLinkedDocuments(
            Document doc)
        {
            return new FilteredElementCollector(doc)
        ...
        private static Level CreateLevel(Document doc, int elevation)
        {
            ...
            var level = Level.Create(doc, elevation); // 2016
        ...
        ///     from them.
        /// </summary>
        private static void BenchmarkAllLevels(Document doc, int nLevels)
        {
            var t = typeof(Level);
            int n;
            ...
            {
                var a
                    = GetNonElementTypeElements(doc).ToElements();
                n = a.Count;
            }
            ...
            {
                var a
                    = GetElementsOfTypeUsingExplicitCode(doc, t);
            ...
            {
                var a =
                    GetElementsOfTypeUsingLinq(doc, t);
            ...
            {
                var a
                    = CollectorGetElementsOfType(doc, t);
            }
            ...
            {
                var a
                    = CollectorGetElementsOfType(doc, t).ToElements();
        ...
        ///     iLevel.
        /// </summary>
        private static void BenchmarkSpecificLevel(Document doc, int iLevel)
        {
            var t = typeof(Level);
            ...
            {
                level = CollectorGetFirstElementOfType(doc, t);
            }
                ...
                "Parameter filter"))
            {
                ...
                level = GetFirstElementOfTypeWithBipString(
                    doc, t, BuiltInParameter.DATUM_TEXT, name);
            }
        ...
        /// </summary>
        private static bool IsElementHiddenInView(
            Element e,
            View v)
        {
            if (v.CropBoxActive)
        ...
        private static void RunBenchmark(Document doc)
        {
            // Create a number of levels for us to play with:

            var maxLevel = 1000;
            for (var i = 3; i < maxLevel; ++i) CreateLevel(doc, i);

            // Run a specified number of tests
            // to retrieve all levels in different
            // ways:

            var nLevels = CollectorGetElementsOfType(doc, typeof(Level))
                .ToElements().Count;
            ...
            using (totalTimer)
            {
                for (var i = 0; i < nRuns; ++i) BenchmarkAllLevels(doc, nLevels);
            }
            ...
            using (totalTimer)
            {
                for (var i = 0; i < nRuns; ++i)
                {
                    ...
                    BenchmarkSpecificLevel(doc, iLevel);
                }
            }
        ...
        /// </summary>
        private static IEnumerable<Element> GetAllExteriorWalls(
            Document doc)
        {
            return new FilteredElementCollector(doc)
                .OfClass(typeof(Wall))
        ...
        /// </summary>
        private static IEnumerable<Element> GetAllModelElements(
            Document doc)
        {
            var opt = new Options();
        ...
        private static IList<Element> GetFamilyInstanceModelElements(
            Document doc)
        {
        ...
        /// </summary>
        private static IEnumerable<Element> SelectAllPhysicalElements(
            Document doc)
        {
            return new FilteredElementCollector(doc)
                .WhereElementIsNotElementType()
        ...
        /// </summary>
        internal static IEnumerable<Element> GetAreasInAreaScheme(
            Document doc,
        ...
        private static void GetDetailCurves(Document doc)
        {
            var collector
                = new FilteredElementCollector(doc);
        ...
        internal static void GetListOfLinestyles(Document doc)
        {
        ...
        private static FilteredElementCollector GetViews(Document doc)
        {
            return new FilteredElementCollector(doc)
                .OfClass(typeof(View));
        }
        ...
        private static ElementId GetSpecificTitleBlockType(Document doc)
        {
        ...
        ///     intersecting a wall.
        /// </summary>
        internal static void GetInstancesIntersectingElement(Element e)
        {
            #region Joe's code
...
+ element.Category.Name + "ID:" + element.Id + ")" );
        ...
        /// </summary>
        private static Solid CollectorGetSolid(Element e)
        {
            return null;
        }
        ...
        private static void F1(Document doc)
        {
            var collector
                = new FilteredElementCollector(doc);
        ...
        private static void F2(Document doc, Level level)
        {
            var collector
                = new FilteredElementCollector(doc);
        ...
        private static void F3LevelDatumTextFilter(Document doc)
        {
            var collector
                = new FilteredElementCollector(doc);
        ...
        private static void F4(Document doc)
        {
        ...
        /// </summary>
        private static Element GetFirstElementOfTypeWithBipString(
            Document doc,
            Type type,
        ...
        {
            var a
                = CollectorGetElementsOfType(doc, type);
        ...
        ///     the element type.
        /// </summary>
        private static List<Element> GetElementsOfTypeUsingExplicitCode(
            Document doc,
            Type type)
        {
            var a
        ...
        ///     the element type.
        /// </summary>
        private static IEnumerable<Element> GetElementsOfTypeUsingLinq(
            Document doc,
            Type type)
        {
            var a
        ...
        /// </summary>
        private static Element GetFirstNamedElementOfTypeUsingExplicitCode(
            Document doc,
            Type type,
            string name)
        {
        ...
        /// </summary>
        private static Element GetFirstNamedElementOfTypeUsingLinq(
            Document doc,
            Type type,
            string name)
        {
        ...
        /// </summary>
        private static Element GetFirstNamedElementOfTypeUsingAnonymousButNamedMethod(
            Document doc,
            Type type,
            string name)
        {
        ...
        ///     type and name using an anonymous method.
        /// </summary>
        private static Element GetFirstNamedElementOfTypeUsingAnonymousMethod(
            Document doc,
            Type type,
            string name)
        {
        ...
        /// </summary>
        private static List<FamilyInstance> GetSortedColumns(Document doc)
        {
            var colums
                = new FilteredElementCollector(doc)
                    .WhereElementIsNotElementType()
    ...
    internal class ColumnMarkComparer : IComparer<FamilyInstance>
    {
        public int Compare(
            FamilyInstance x,
            FamilyInstance y)
