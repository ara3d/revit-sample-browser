...
namespace Revit.SDK.Samples.Toposolid.CS
{
   ...
   public class ToposolidCreation : IExternalCommand
   {
         ...
         , ref string message, ElementSet elements)
      {
         try
         {
            ...
            var typeId = new FilteredElementCollector(doc).OfClass(typeof(ToposolidType)).OfType<ToposolidType>().FirstOrDefault()?.Id;
            if (typeId == null)
            {
   ...
   public class ToposolidFromDWG : IExternalCommand
   {
      ...
      public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
      {
         ...
         var element = doc.GetElement(sel.PickObject(ObjectType.Element, new ImportInstanceFilter(), "pick an imported dwg file").ElementId);
   ...
   public class ContourSettingCreation : IExternalCommand
   {
      ...
      {
         var doc = commandData.Application.ActiveUIDocument.Document;
         var topoType = new FilteredElementCollector(doc).OfClass(typeof(ToposolidType)).OfType<ToposolidType>().FirstOrDefault();
         if (topoType == null)
         {
            TaskDialog.Show("Error", "Can not find a valid ToposolidType");
            return Result.Failed;
   ...
   public class ToposolidFromSurface : IExternalCommand
   {
      ...
      public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
      {
         ...
         var surface = doc.GetElement(sel.PickObject(ObjectType.Element, new TopographySurfaceFilter(), "pick a topography surface")) as TopographySurface;
   ...
   public class SplitToposolid : IExternalCommand
   {
      ...
      public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
      {
         ...
         var topo = doc.GetElement(sel.PickObject(ObjectType.Element, new ToposolidFilter())) as Autodesk.Revit.DB.Toposolid;
