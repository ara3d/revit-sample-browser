...
3. Command markdown docs under `src/<Sample>/<slug>.md` are the canonical reference for each sample.
...
- **Sources:** read each command's `.md` doc, then its `.cs` source; consult nested helpers only when needed.
